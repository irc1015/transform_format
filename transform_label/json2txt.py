...
#classes = ["person", "bike", "car", "motor", "bus", "truck", "rider", "train"]#original classes
car_same_calss = ["bike", "car", "motor", "bus", "truck", "rider", "train"]#merge these classes into a class
new_class = ["person", "car"]#new classes
...
def convert(image_name, label):
    ...
    classIndex = new_class.index(catName)
    roi = label['box2d']
...
if __name__ == '__main__':
    ...
    label_File_Path = [os.path.join(args.json_path, TRAIN_LABEL_NAME), os.path.join(args.json_path, VAL_LABEL_NAME)]
